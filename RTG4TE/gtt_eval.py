...
def score(mapping, pred, gold):
    ...
    # spurious当预测的事件类型与黄金标注的事件类型不一致，统计预测错误的实体个数
...
def eval_tf(preds, golds):
    ...
    for docid in docids:
        pred = preds[docid]
        gold = golds[docid]
        K, V = list(range(len(pred))), list(range(len(gold)))#len(pred)表示预测的事件个数和len(gold)表示黄金标注的事件个数，并生成列表eg：K：[0, 1] V：[0, 1, 2, 3, 4]
        V = V + [-1]#在黄金标注的事件序号中添加-1，保证当预测的事件个数大于黄金标注的事件数时，能够成功匹配。
        print(K, V)
        init_maps = [dict(zip(K, p)) for p in itertools.product(V, K)]
#len(init_maps)=len(k)个len(v)相乘
        ex_best = None
        map_best = None
        for p in itertools.product(V, repeat=len(K)):
            mapping = dict(zip(K, p))
        for mapping in init_maps:
            if not is_valid_mapping(mapping):#找到存在的匹配方式
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_file", default="preds_gtt.out", type=str, required=False, help="preds output file")
