...
class CopyBartForConditionalGeneration(BartForConditionalGeneration):
    ...
    def __init__(self, config: BartConfig, model_dim = 768):
        super().__init__(config)
        
        self.selected_heads = None
        self.linear_copy = torch.nn.Linear(model_dim, 1)
    ...
    def forward(
        ...
        cross_attention_non_linear = self.model.decoder.layers[-1].encoder_attn.out_proj.weight # (emb_dim, emb_dim)[768, 768]
        ...
        # cross_attentions_aggregate = last_cross_attentions[:, :, :, :].mean(dim=1)
        ...
        copy_logits = torch.zeros_like(lm_logits) #
        copy_logits.scatter_add_(dim=2, index=dummy_input_ids, src=cross_attentions_aggregate)
        p_gen = self.linear_copy(decoder_last_hidden_state)
