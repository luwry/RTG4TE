...
class GenerativeModel(nn.Module):
    def __init__(self,
        ...
        self._k = config.k
        # self.loss_fct = torch.nn.functional.binary_cross_entropy(reduction='mean')
    ...
    def forward(self, batch, decoder_input_ids=None, decoder_labels=None, decoder_masks=None, logger=None, tag=None, step=None, tokenizer=None):
        ...
        if self.use_copy or self.use_SAGCopy:
            bart_outputs = self.encode(batch, decoder_input_ids=decoder_input_ids, decoder_labels=decoder_labels)#模型输出
        else:
    ...
    def beam_search(self, batch, num_beams, decoding_length):
        '''
    ...
    def predict(self, batch, tokenizer, epoch=None):
        ...
        with torch.no_grad():
            ...
            decoder_token_masks[:, tokenizer.convert_tokens_to_ids(ROLES)] = 1
            decoder_token_masks[:, tokenizer.eos_token_id] = 1
            decoder_token_masks[:, tokenizer.bos_token_id] = 1
            for role in ROLES:
                decoder_token_masks[:, tokenizer.encode(role, add_special_tokens=True)]  = 1
            ...
            if self.decoding_method == 'greedy':
                ...
                decoded_ids = self.beam_search(batch, num_beams=4, decoding_length=decoding_length)
            else:
