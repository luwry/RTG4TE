...
from transformers import AutoTokenizer, AdamW, get_linear_schedule_with_warmup, Adafactor, AutoModelForSeq2SeqLM
...
parser.add_argument('-c', '--config', default='config/tf_generative_model.json')
...
tokenizer.add_tokens(ROLES)


print('==============Prepare Training Set=================')
train_set = IEataset(config, config.train_file, tokenizer, max_length=config.max_length, gpu=use_gpu)
print('==============Prepare Dev Set=================')
dev_set = IEDataset(config, config.dev_file, tokenizer, max_length=config.max_length, gpu=use_gpu)
print('==============Prepare Test Set=================')
test_set = IEDataset(config, config.tes_file, tokenizer, max_length=config.max_length, gpu=use_gpu)
vocabs = {}

if config.task == "tf":
    tf_dev = read_tf_gold_file(config.grit_dev_file)
    tf_test = read_tf_gold_file(config.grit_test_file)
elif config.task == "ree":
    grit_dev = read_grit_gold_file(config.grit_dev_file)
    grit_test = read_grit_gold_file(config.grit_test_file)
# grit_test = read_gold_file(config.grit_test_file)
...
for epoch in range(config.max_epoch):
    ...
    optimizer.zero_grad()
    train_gold_outputs, train_pred_outputs, train_input_tokens, train_doc_ids, train_input_ids, _idx, _loss = [], [], [], [], [], [], []
    training_loss = 0
    for batch_idx, batch in enumerate(DataLoader(train_set, batch_size=config.batch_size, shuffle=True, drop_last=False, collate_fn=train_set.collate_fn)):
    ...
    if config.max_epoch <= 50 or epoch % (config.max_epoch // 150) == 0 :
        ...
        if config.task == "ree":
            ree_preds = construct_outputs_for_ree(dev_pred_outputs, dev_input_tokens, dev_doc_ids, tokenizer)
            dev_scores = ree_eval.ree_eval(ree_preds, grit_dev)
        elif config.task == "tf":
            tf_preds = construct_outputs_for_tf(dev_pred_outputs, dev_input_tokens, dev_doc_ids, tokenizer)
            dev_scores = tf_eval.eval_tf(tf_preds, grit_dev)
        ...
        if save_model:
            ...
            if config.task == "ree":
                ree_preds = construct_outputs_for_ree(test_pred_outputs, test_input_tokens, test_doc_ids, tokenizer)
                test_scores = ree_eval.ree_eval(ree_preds, grit_test)
            elif config.task == "tf":
                tf_preds = construct_outputs_for_tf(test_pred_outputs, test_input_tokens, test_doc_ids, tokenizer)
                test_scores = tf_eval.eval_tf(tf_preds, grit_test)
